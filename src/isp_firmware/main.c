...
/* Vector tables is first in linker script. */
extern uint32_t RAM_START_ADDRESS;

void init(void) {
  /*  Set vector table offset, required since we are not use the eSRAM
   *  re-map feature.
   */
  uint32_t vector_address = (uint32_t)&RAM_START_ADDRESS;
  HW_set_32bit_reg(0xE000ED08u, vector_address);
  /* Init uart */
  InitUART();
}
